<script>
    ...
    $("#paymentcode").on('input', function() {
        ...
        if ($('#paymentno option').filter(function() {
            ...
            $.ajax({
                type: 'post',
                url: "{{ url('Session-Id/cpaymentid')}}" + '/' + urlid,
                success: function() {
                    PayMentInfo();
                }
            });
    ...
    function PayMentInfo() {
        $.ajax({
            type: "get",
            url: "{{ url('CustomerPayment/getView')}}",
            datatype: ("json"),
            success: function(data) {
                urlid = data.id
    ...
    $(document).on('click', "#pdf", function() {
        url = "{{ url('CustomerPayment/pdf')}}" + '/' + urlid,
            window.open(url, '_blank');
    })
    $("#mail").on('click', function() {
    if (urlid > 0) {
      url = "{{ url('CustomerPayment/sendmail')}}" + '/' + urlid,
        window.location = url;
    }
  });
        ...
        swal({
            ...
            .then((willDelete) => {
                if (willDelete) {
                    ...
                    $.ajax({
                        type: "post",
                        url: "{{ url('CustomerPayment/delete')}}" + '/' + urlid,
                        success: function(data) {
