<script type="text/javascript">
    ...
    function InvoiceCode() {
        $.ajax({
            type: 'get',
            url: "{{ route('creditinvoice.invoicecode') }}",
            datatype: 'JSON',
            success: function(data) {
                invoiceno = data;
    ...
    /*     $('#customersearch').on('keyup', function() {
            ...
            if (value == "") {
                ...
                $.ajax({
                    ...
                    url: "{{ route('customer.customercearchdatalist') }}",
                    datatype: 'JSON',
                    success: function(data) {
                        $('#customer').html(data);
                    },
            ...
            if ($('#customer option').filter(function() {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        $("#creditinv").val(credit);
                        $("#cuspayment").val(data.payment);
                        $("#cusdiscount").val(data.discount);
    ...
    /* function cusloadData(data) {
        credit = data.credit;
        payment = data.payment;
        dicount = data.totaldiscount;
        balancedue = data.totaldiscount;
        $("#creditinv").val(credit);
        $("#cuspayment").val(payment);
        $("#cusdiscount").val(dicount);
        $("#balancedue").val(balancedue);
    } */
        ...
        swal("Successfully Data Save", "Data Submit", "success", {
            ...
            .then((value) => {
                switch (value) {
                    ...
                    case "Show":
                        url = "{{ url('Credit-Invoice/show')}}" + '/' + data,
                            window.open(url, '_blank');
                        break;
                    case "catch":
                        url = "{{ url('Credit-Invoice/LoadPrintslip')}}" + '/' + data,
                            window.open(url, '_blank');
                        break;
                    case "datapdf":
                        url = "{{ url('Credit-Invoice/invoicepdf')}}" + '/' + data,
                            window.open(url, '_blank');
                        break;
