    ...
    $("#customersearch").on('input', function() {
        ...
        if (val == "") {
            ...
            if ($('#customer option').filter(function() {
                ...
                $.ajax({
                    type: "get",
                    url: "{{ url('Customer/getAmounthistory')}}" + '/' + customerid,
                    datatype: ("json"),
                    success: function(data) {
                        loadData(data);
                    },
        ...
        if (type == 1) {
            ...
            if (recieve == 0 || customerid == 0 || nbalancedue == "" || nbalancedue < 0) {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        $("#overlay").fadeOut();
                        url = "{{ url('CustomerPayment/paypalprocess')}}" + '/' + data,
                            window.open(url);
                            ExecuteClear();
                    },
                    error: function(data) {
        ...
        swal("Successfully Data Save", "Data Submit", "success", {
            ...
            .then((value) => {
                switch (value) {
                    ...
                    case "Show":
                        url = "{{ url('CustomerPayment/show')}}" + '/' + data,
                            window.location = url;
                        break;
                    case "catch":
                        //PurchasePrint();
                        break;
                    case "datapdf":
                        url = "{{ url('CustomerPayment/pdf')}}" + '/' + data,
