...
<script>
    ...
    $("#vatcollection").on('input', function() {
        ...
        if ($('#vatno option').filter(function() {
            ...
            $.ajax({
                type: "post",
                url: "{{ url('Vat-Collections/setVatcollectionId')}}" + '/' + id,
                datatype: ("json"),
                success: function(data) {
                    VatPaymentDetails();
                },
    ...
    function VatPaymentDetails() {
        $.ajax({
            type: "get",
            url: "{{ url('Vat-Collections/getView')}}",
            datatype: ("json"),
            success: function(data) {
                vatid = data.id;
    ...
    $(document).on('click', '#pdfdata', function() {
        url = "{{ url('Vat-Collections/pdf')}}" + '/' + vatid,
            window.open(url, '_blank');
    });
    $(document).on('click', '#print', function() {
        url = "{{ url('Vat-Collections/LoadPrintslip')}}" + '/' + vatid,
            window.open(url, '_blank');
    });
            ...
            swal({
                ...
                .then((willDelete) => {
                    if (willDelete) {
                        ...
                        $.ajax({
                            type: "post",
                            url: "{{ url('Vat-Collections/delete')}}" + '/' + vatid,
                            success: function() {
