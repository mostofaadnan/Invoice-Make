<script>
    ...
    $("#returncode").on('input', function() {
        ...
        if ($('#returnnumber option').filter(function() {
            ...
            $.ajax({
                type: 'post',
                url: "{{ url('Session-Id/salereturnid')}}" + '/' + returnid,
                success: function() {
                    returnDetails();
                }
            });
    ...
    $("#returnpdf").on('click', function() {
        url = "{{ url('Sale-Return/returnpdf')}}" + '/' + returnid,
            window.open(url, '_blank');

    });
    $(document).on('click', '#printslip', function() {

        url = "{{ url('Sale-Return/LoadPrintslip')}}" + '/' + returnid,
            window.open(url, '_blank');
    });
    $("#mail").on('click', function() {
        if (returnid > 0) {
            url = "{{ url('Sale-Return/sendmail')}}" + '/' + returnid,
                window.location = url;
        }
            ...
            swal({
                ...
                .then((willDelete) => {
                    if (willDelete) {
                        ...
                        $.ajax({
                            type: "post",
                            url: "{{ url('Sale-Return/delete')}}" + '/' + returnid,
                            success: function() {
