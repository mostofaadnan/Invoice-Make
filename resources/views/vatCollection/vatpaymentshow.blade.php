...
<script>
    ...
    $("#paymentcode").on('input', function() {
        ...
        if ($('#paymentno option').filter(function() {
            ...
            $.ajax({
                type: "post",
                url: "{{ url('Vat-Collections/SetPaymentId')}}" + '/' + paymentid,
                datatype: ("json"),
                success: function(data) {
                    VatPaymentDetails();
                },
    ...
    function VatPaymentDetails() {
        $.ajax({
            type: "get",
            url: "{{ url('Vat-Collections/getPaymentView')}}",
            datatype: ("json"),
            success: function(data) {
                pid = data.id;
                lodData(data);
    ...
    $(document).on('click', '#pdfdata', function() {
        url = "{{ url('Vat-Collections/vatPaymentPdf')}}" + '/' + pid,
            window.open(url, '_blank');
    });
    $(document).on('click', '#print', function() {
        url = "{{ url('Vat-Collections/LoadPrintslip')}}" + '/' + pid,
            window.open(url, '_blank');
    });
