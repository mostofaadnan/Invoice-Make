...
class CustomerController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $customer->tell_no = $request->tell_no;
        $customer->fax_no = $request->fax_no;
        $customer->email = $request->supemail;
        $customer->website = $request->website;
        $customer->openingDate = $request->openingdate;
        ...
        if ($customer->save() == true) {
            ...
            if (!is_null($multicategory)) {
                foreach ($multicategory as $mcat) {
                    // $catid = Category::Where('title', $mcat)->first();
                    $multycate = new MultiselectCategory();
                    $multycate->category_id = $mcat;
                    $multycate->type = "Customer";
                    $multycate->parent_id = $customer->id;
                    $multycate->save();
                }
            }
